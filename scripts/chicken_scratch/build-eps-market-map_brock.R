...
myzip <- readShapePoly(file.path(data_dir,'eps_market',"US_Zip_-_Lower_48.shp"))

## read in the eps markets
#ch <- odbcConnectExcel(file.path(eps_data_dir,'eps.xls'))
...
eps <- read_excel(path = file.path(eps_data_dir,'eps.xls'), sheet = 'Sheet 1', col_names = TRUE)
eps %>% str()
