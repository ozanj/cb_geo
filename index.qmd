...
insert_figure <- function(rq, metro, graph_type, base_path = graphs_dir) {
  ...
  # 'metro' is already underscored (e.g. "los_angeles").
  # We'll use it directly for file paths, but if you want
  # to be extra safe, you can do:
  # metro_underscored <- str_replace_all(metro, " ", "_")
  # ... (assuming it has no spaces).
...
for (m in metros) {
  ...
  # Convert "los_angeles" -> "Los Angeles" for display in slides
  heading_text <- gsub("_", " ", m, fixed = TRUE)
  heading_text <- tools::toTitleCase(heading_text)

  # Print a top-level heading for the metro area in user-friendly form
  writeLines(str_c("## ", heading_text, " area"))
  ...
  for (rq in rqs) {
    ...
    if (rq == "rq1") {
      ...
      # Use 'm' for the file path to the map (underscored version).
      # If you had a special case for "los_angeles", you could handle that here.
      rq1_map <- str_c('./results/maps/rq1_map_', m, '.html')
      
      # But let's illustrate one special example for "los_angeles":
      if (m == "los_angeles") {
        ...
        for (rc in c("row", "col")) {
          ...
          if (rc == "row") {
            ...
            for (order_id in orders_for_m) {
              ...
              # e.g. "race_col_plot_448375_546954"
              # => "rq2_los_angeles_race_col_plot_448375_546954.png"
