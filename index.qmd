...
- Correlations from training data caused by structural inequality
...
- Analyze purchases that include all Geomarkets in a metro area
  - Show who would be included/excluded had search criteria filtered on particular Geomarket(s)
...
for (m in metros) {
  ...
  for (rq in rqs) {
    ...
    if (rq == "rq1") {
      ...
      rq1_map <- if_else(
        m == 'los angeles',
        'https://rq1-map-los-angeles.netlify.app',
        str_c('./results/maps/rq1_map_', str_replace_all(m, " ", "_"), '.html')
      )
    ...
    # ---------------------------------------------
      ...
      writeLines(str_c('<iframe data-src="./results/maps/rq2_map_', str_replace_all(m, " ", "_"), '.html" src="about:blank" width=100% height=100% allowtransparency="true"></iframe>'))
      writeLines('<p class="fullscreen">Fullscreen</p>')
...
- Poor, non-white Geomarkets produce disproportionate share of Black, Hispanic (and often Asian) students
...
- Scholarship focuses on schools and policies as "sorting machines" [@RN4873]
  ...
  - College access structured by third-party vendors, products that amplify structural inequalities observed on demand-side
